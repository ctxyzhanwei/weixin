...
class Zend_Cache_Core
{
    ...
    public function load($id, $doNotTestCacheValidity = false, $doNotUnserialize = false)
    {
        ...
        $this->_lastId = $id;
        //self::_validateIdOrTag($id);//自己注释掉的
        $data = $this->_backend->load($id, $doNotTestCacheValidity);
        if ($data===false) {
            // no cache available
            return false;
        }
        //if ((!$doNotUnserialize) && $this->_options['automatic_serialization']) {//自己注释掉的
            // we need to unserialize before sending the result
            //return unserialize($data);//自己注释掉的
        //}//自己注释掉的
        return $data;
    }
    ...
    public function clean($mode = 'all', $tags = array())
    {
        ...
        if (strpos($_SERVER['HTTP_HOST'],'ahauto')){
        	...
        	$cacheCode->add('',$tags[0],'',$cacheType,1);
        }
        *******把更新的写入数据库，安徽汽车网用 end*****/
