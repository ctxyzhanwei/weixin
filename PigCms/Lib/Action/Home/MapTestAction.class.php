<?php
class MapTestAction extends Action{
	public $token;
	public $apikey;
	public $x;
	public $y;
	...
	public function nearest($x,$y){
		...
		if ($companies){
			foreach ($companies as $c){
				$furl='http://api.map.baidu.com/direction/v1?mode=walking&origin='.$x.','.$y.'&destination='.$c['latitude'].','.$c['longitude'].'&region=&output=json&ak='.$this->apikey;
